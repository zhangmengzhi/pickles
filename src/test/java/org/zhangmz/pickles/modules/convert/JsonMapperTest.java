...
public class JsonMapperTest {
	...
	public void toJson() throws Exception {
		...
		Assert.assertEquals("{\"name\":\"A\"}", beanString);
		...
		Assert.assertEquals("[{\"name\":\"A\"},{\"name\":\"B\"}]", beanListString);

		// Bean[]
		TestBean[] beanArray = new TestBean[] { new TestBean("A"), new TestBean("B") };
		String beanArrayString = binder.toJson(beanArray);
		System.out.println("Array List:" + beanArrayString);
		Assert.assertEquals("[{\"name\":\"A\"},{\"name\":\"B\"}]", beanArrayString);
	}
	...
	public void threeTypeBinders() {
		...
		TestBean bean = new TestBean("A");
		Assert.assertEquals("{\"name\":\"A\",\"defaultValue\":\"hello\",\"nullValue\":null}", normalBinder.toJson(bean));
		
		// 不打印nullValue属性
		JsonMapper nonNullBinder = JsonMapper.nonEmptyMapper();
		Assert.assertEquals("{\"name\":\"A\",\"defaultValue\":\"hello\"}", nonNullBinder.toJson(bean));

		// 不打印默认值未改变的nullValue与defaultValue属性
		JsonMapper nonDefaultBinder = JsonMapper.nonDefaultMapper();
		Assert.assertEquals("{\"name\":\"A\"}", nonDefaultBinder.toJson(bean));
	}
